...
def count_calls():   #Создать функцию (def) count_calls и изменять в ней значение переменной calls.
    global calls     #Изменяем (global) глобальную переменную внутри функции.
    calls += 1       #Изменяем значение переменной (calls = 0 + 1)

def string_info (string):   #Создать функцию string_info с параметром string.
    count_calls()           #Первая функция, вызывается в остальных двух функциях.
    return (len(string), string.upper(), string.lower())    #принимает(return) аргумент - строку(string) и возвращает(return)
                                                            #кортеж() из: длины этой строки,
                                                            #строку в верхнем регистре, строку в нижнем регистре.

def is_contains (string, list_to_search):     #Создать функцию is_contains с двумя параметрами string и list_to_search.
    count_calls()                             #Первая функция, вызывается в остальных двух функциях.
    return string.upper() in [s.upper() for s in list_to_search]    #принимает(return) два аргумента: строку и список,
